...
!!!!!
  ...
  USE epwcom,        ONLY : tdboltzmann, tstep, Ntstep, tstep_write, temp_el, temp_ph,  &
                            initial_type, initial_sigma, ef_c, ef_v,                    &
                            carrier_dynamic, double_grid, nkf1d, nkf2d, nkf3d, nqf1d,   &
                            nqf2d, nqf3d, dph, bte_restart, bte_phph, bte_elel,         &
                            bte_solver, ph_read, ph_rta, facbroad, phwmin, teph_max,    &
                            ephmat_dir, lscreen_tdbe, static_scr, mode_select,          &
                            phself_anh, two_ef, lorentz_gamma, phlw_chp, phphself_r                            
  ! -------------------------------------------------------------------------------------
  ...
  ! -------------------------------------------------------------------------------------
  !Added for calculating time-dependent Boltzmann transport Equation
       tdboltzmann, tstep, Ntstep, tstep_write, temp_el, temp_ph,              &
       initial_type, initial_sigma, ef_c, ef_v,                                &
       carrier_dynamic, double_grid, nkf1d, nkf2d, nkf3d, nqf1d,               &
       nqf2d, nqf3d, dph, bte_restart, bte_phph, bte_elel,                     &
       bte_solver, ph_read, ph_rta, facbroad, phwmin, teph_max,                &
       ephmat_dir, lscreen_tdbe, static_scr, mode_select, phself_anh, two_ef,  &
       lorentz_gamma, phlw_chp, phphself_r                          
  ! -------------------------------------------------------------------------------------    
  ! --------------------------------------------------------------------------------
  !
  ! amass    : atomic masses
  ...
  ahc_win_max    = -9999.d0
  ! ---------------------------------------------------------------------------------
  ! Input parameters of time-dependent BTE
  tdboltzmann = .false.
  tstep = 1.0
  Ntstep = 3000
  tstep_write = 30
  ...
  !
  IF (tdboltzmann .AND. .NOT. double_grid) THEN
    nqf1d        = nqf1
    nqf2d        = nqf2
    nqf3d        = nqf3
    nkf1d        = nkf1
...
101   CALL errore('epw_readin', 'opening file ' // filqf, ABS(ios))
  ...
  ephmat_dir = trimcheck(ephmat_dir)
  !

  ...
  CONTAINS
    ...
    !
    !--------------------------------------------------------------------------
    SUBROUTINE compute_dw(first_q, ik, nrr, cuf, etf, etf_ks, cfac, &
                                    dims, uf_ph, epmatf, dwf, xxq, rrf)
    !--------------------------------------------------------------------------
    !! Compute DW term truncated to the active space at ik.
    !--------------------------------------------------------------------------
    USE kinds,         ONLY : DP
    ...
    USE modes,         ONLY : nmodes
    USE epwcom,        ONLY : nbndsub, ahc_win_min, ahc_win_max
    USE elph2,         ONLY : ibndmin, nbndfst, dwmatwe
    USE wan2bloch,     ONLY : dmewan2bloch, dwwan2blochp, ephwan2bloch
    USE wfpt_mod,      ONLY : dwmatf_trunc
    !
    IMPLICIT NONE
    !
    ...
    COMPLEX(KIND = DP), INTENT(inout) :: dwf(nbndfst, nbndfst, 3, nmodes)
    !! Truncated Debye-Waller matrix element in electron Bloch basis, phonon Cartesian basis
    COMPLEX(KIND = DP) :: dwmatf(nbndsub, nbndsub, 3, nmodes)
    ...
    IF (first_q) THEN
      !
      CALL ephwan2bloch(nbndsub, nrr, dwmatwe, cuf, cuf, dwmatf, &
            3*nmodes, cfac, dims, xxq, rrf, longrange = .FALSE.)
      dwmatf_trunc(:, :, :, :, ik) = dwmatf
!       CALL dmewan2bloch(nbndsub, nrr, cuf, pmef, etf, etf_ks, cfac, dims, use_momentum = .TRUE.)
!       !
!       DO pb = 1, nbndsub
!         !
...
!       ENDDO
    ...
    !
!     CALL dwwan2blochp(nbndfst, dwmatf_trunc(:, :, :, :, ik), uf_ph, dwf, nmodes)
    CALL dwwan2blochp(nbndsub, dwmatf_trunc(:, :, :, :, ik), uf_ph, dwf, nmodes)
    !
    !--------------------------------------------------------------------------
    END SUBROUTINE compute_dw
    !--------------------------------------------------------------------------
    !
    !--------------------------------------------------------------------------

  ...
  USE selfen_ahc,       ONLY : selfen_elec_ahc_active, compute_dw_truncated,       &
                               ahc_run_static_wfpt, selfen_print, compute_dw
#if defined(__MPI)
  ...
  IF (.NOT. epmatkqread) THEN
    ...
    IF (phonselfen) THEN
      ...
      ALLOCATE(domega_all(nmodes, totq, nsmear, nstemp), STAT = ierr)
      IF (ierr /= 0) CALL errore('ephwann_shuffle', 'Error allocating domega_all', 1)
      lambda_all(:, :, :, :)   = zero
      lambda_v_all(:, :, :, :) = zero
      gamma_all(:, :, :, :)    = zero
      gamma_v_all(:, :, :, :)  = zero
        ...
        DO iqq = iq_restart, totq
          ...
          DO ik = 1, nkf
            ...
            IF (wfpt) THEN
              ...
              ELSE
                IF (phonselfen) &
                CALL compute_dw(first_q, ik, nrr_k, cufkk, etf(:, ikk), &
                 etf_ks(:, ikk), cfac, dims, uf, epmatf, dwf17(:, :, :, ik), xxq, rrf(:, :, :))
                  !
              ENDIF
              !
            ENDIF ! wfpt
    ...
    IF (phonselfen) THEN
      ...
      DEALLOCATE(domega_all, STAT = ierr)
      IF (ierr /= 0) CALL errore('ephwann_shuffle', 'Error deallocating domega_all', 1)
    ENDIF
    IF (specfun_el .OR. specfun_pl) THEN

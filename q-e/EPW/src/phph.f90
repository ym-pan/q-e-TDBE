...
!!      It is simplified to the expression below
    ...
    !! inverse of broadening parameter
    INTEGER :: ierr
    !! Error status
    INTEGER :: ios 
    !! Error status
          ...
          DO iq2 = 1, nqtotf
            DO nu2 = 1, nmodes 
              ...
              DO nu3 = 1, nmodes
                ...
                wqnu3p = wf(nu3, iq3p)
                !
                IF (wqnu3p >= phwmin .AND. ABS(wqnu1 + wqnu2 - wqnu3p) <= (2.d0 * degaussq)) THEN
                  CALL psi_interp(nu1, nu2, nu3, iq1, iq2, iq3p,  1.0_dp, psi2p)
                  !
                  psi2p = psi2p / 4.d0 / nqtotf 
                  !! The unit of psi2p and psi2m is [Ry]^2
                  !
                  DO itemp = 1, ntemps
                    temp_in_ry = (temp_min + (temp_max - temp_min) / (ntemps - 1)  &
                    ...
                    nph2  = 1.0 / (EXP(wqnu2 / temp_in_ry) - 1.0)
                    nph3p = 1.0 / (EXP(wqnu3p / temp_in_ry) - 1.0)
                    weight = w0gauss((wqnu1 + wqnu2 - wqnu3p) * inv_degaussq , 0) * inv_degaussq
                    pi_anh(itemp, iq1, nu1) = pi_anh(itemp, iq1, nu1) + pi * weight * psi2p * (nph2 - nph3p)
                  ENDDO
                ENDIF
                !
                wqnu3m = wf(nu3, iq3m)
                !
                IF (wqnu3m >= phwmin .AND. ABS(wqnu1 - wqnu2 - wqnu3m) <= (2.d0 * degaussq)) THEN
                  CALL psi_interp(nu1, nu2, nu3, iq1, iq2, iq3m, -1.0_dp, psi2m)
                  psi2m = psi2m / 4.d0 / nqtotf 
                  !! The unit of psi2p and psi2m is [Ry]^2
                  !
                  DO itemp = 1, ntemps
                    temp_in_ry = (temp_min + (temp_max - temp_min) / (ntemps - 1)  &
                    ...
                    nph2  = 1.0 / (EXP(wqnu2 / temp_in_ry) - 1.0)
                    nph3m = 1.0 / (EXP(wqnu3m / temp_in_ry) - 1.0)
                    weight = w0gauss((wqnu1 - wqnu2 - wqnu3m) * inv_degaussq , 0) * inv_degaussq
                    pi_anh(itemp, iq1, nu1) = pi_anh(itemp, iq1, nu1) + pi * weight * psi2m * (nph2 + nph3m + 1.d0) / 2.d0
                  ENDDO
                ENDIF
                !
              ENDDO ! nu3
    ...
    IF (mpime == ionode_id) THEN
      DO itemp = 1, ntemps
        ...
        WRITE(temp_in_ch, "(I0)") NINT(temp_in_ry)
        IF (phphself_r) THEN
          fname = "pi_anh_real" // TRIM(adjustl(temp_in_ch)) // "K"
        ELSE
          fname = "pi_anh_img" // TRIM(adjustl(temp_in_ch)) // "K"
        ENDIF
        OPEN(unit = 222, FILE = fname)
        DO iq1 = 1, nqtotf
          DO nu1 = 1, nmodes

...
!    USE constants_epw, ONLY : twopi, ci, zero, eps6, ryd2ev, czero
    ...
    USE constants_epw, ONLY : zero, ryd2ev, eps8
    USE mp_global,     ONLY : inter_pool_comm, npool, my_pool_id
    USE mp_world,      ONLY : mpime
...
!      WRITE(stdout, '(5x, a, ES20.10)') 'Fermi level from nscf calculation (eV) = ', ef*ryd2ev
    ...
    !
      ...
      USE epwcom,        ONLY : mp_mesh_k, bte_phph, bte_elel
...
!     IF (ierr /= 0) CALL errore('crystal_fmt_read', 'Error allocating w_centers', 1)
    ...
    USE pwcom,     ONLY : ef
    USE elph2,     ONLY : chw, rdw,zstar, epsi, adapt_smearing, cvmew,ifc
    ...
    INTEGER :: ierr
    !! Error status
    COMPLEX(KIND = DP) :: dummy
    !! Position matrix element in wannier basis, but not needed in TDBE calculations
    !
    ...
    IF (adapt_smearing) THEN
      ...
      IF (vme == 'wannier' .AND. adapt_smearing) THEN
        IF (mpime == ionode_id) THEN
      ...
      WRITE(stdout, '(/5x,"Finished reading vmedatadata.fmt"/)')
    ENDIF
    !
    !------------------------------------------------------------------------------

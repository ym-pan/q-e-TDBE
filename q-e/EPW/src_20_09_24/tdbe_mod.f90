  ...
  !-------------------------------------------------------------------------
  !!
  !! This module contains all the routines variables related to
  ...
  CONTAINS
    ...
    USE epwcom,        ONLY : nkf1, nkf2, nkf3, nqf1, nqf2, nqf3, &
                              ...
                              nqf3d
    USE tdbecom,       ONLY : indx_mapk, indx_mapq  
    !
    IMPLICIT NONE
    !
    INTEGER :: ierr
    ...
    USE eliashbergcom, ONLY : ixkff, xkfs, nkfs
    USE tdbecom,       ONLY : ikfsdx, nkfsden, indx_mapk
    ...
    !-----------------------------------------------------------------------
    USE kinds,         ONLY : DP
    USE elph2,         ONLY : nqf, xqf, xkf, nkf, nqtotf, nktotf, &
                              nbndfst
    USE tdbecom,       ONLY : enk_all, totq, selecq  
...
!    USE constants_epw, ONLY : twopi, ci, zero, eps6, ryd2ev, czero
    USE epwcom,        ONLY : fsthick, mp_mesh_k, restart_step
    USE pwcom,         ONLY : ef
    !
    IMPLICIT NONE
    !
    ...
    USE epwcom,        ONLY : nqf1d, nqf2d, nqf3d, fsthick,nkf1d,    &
                              nkf2d, nkf3d, degaussw, eps_acustic                     
    USE constants_epw, ONLY : zero, ryd2ev, eps8, one, two, czero
    USE mp_global,     ONLY : inter_pool_comm, npool, my_pool_id
    USE mp_world,      ONLY : mpime
    USE mp,            ONLY : mp_bcast, mp_barrier, mp_sum
    USE eliashbergcom, ONLY : dosef,efm0 => ef0, ekfs, wkfs,ixkf, g2, &
                              ...
                              ibnd_kfs_all_to_kfs
    USE tdbecom,       ONLY : iq2nqfs
    USE division,      ONLY : fkbounds
    USE low_lvl,       ONLY : set_ndnmbr, mem_size_eliashberg
    USE epwcom,        ONLY : mp_mesh_k, double_grid, ephmat_dir,     &
                              lscreen_tdbe, scr_typ 
    USE elph2,         ONLY : epsi
    USE rigid_epw,     ONLY : rpa_epsilon, tf_epsilon
    !
    IMPLICIT NONE
    !
...
!    !! Lower/upper bound index for .ephmat file read in current pool
    ...
    IF (mpime == ionode_id) THEN
      ! IF (double_grid) THEN
      ! OPEN(UNIT = iuselecqfd, FILE = 'selecqfd.fmt', STATUS = 'old', IOSTAT = ios)
      ...
      IF (ios /= 0) CALL errore('read_g2_tdbe', 'error opening file ' // filfreq, iufilfreq)

      READ(iufilfreq) nqtotfd, nqf1d_, nqf2d_, nqf3d_, nmode
      IF (nqtotfd /= nqf1d * nqf2d  * nqf3d) &
        CALL errore('read_g2_tdbe', 'e-ph mat elements were not calculated on the nqf1d, nqf2d, nqf3d mesh', 1)
    !
      npool_g2=0
    ...
    IF (mpime /= ionode_id) ALLOCATE(selecqfd(totqfd))
    CALL mp_bcast(selecqfd, ionode_id, inter_pool_comm)
    CALL mp_bcast(nqtotfd, ionode_id, inter_pool_comm)
    CALL mp_bcast(nmode, ionode_id, inter_pool_comm)
    ...
    !
    ALLOCATE(wfd(nmode, nqtotfd), STAT = ierr)
    ...
    IF (mpime == ionode_id) THEN
      DO iqq = 1, totqfd ! loop over q-points in fsthick
        iq = selecqfd(iqq)
        !
        READ(iufilfreq) xqfd(:, iq)
        DO imode = 1, nmode
          READ(iufilfreq) wfd(imode, iq)
        ENDDO
      ENDDO
      CLOSE(iufilfreq)
    ENDIF
    ! first node broadcasts everything to all nodes
    CALL mp_bcast(wfd, ionode_id, inter_pool_comm)
    CALL mp_bcast(xqfd, ionode_id, inter_pool_comm)
    CALL mp_barrier(inter_pool_comm)
...
!      WRITE(stdout, '(5x, a, ES20.10)') 'Fermi level from nscf calculation (eV) = ', ef*ryd2ev
    ...
    IF ((ebndmax1 > eps8) .OR. (ebndmax2 > eps8)) THEN
      CALL errore('read_g2_tdbe', 'Error: ekfs_all is not equal to ekfs.', 1)
    ENDIF
    !
    WRITE(stdout,'(/5x,a/)') 'Start reading e-ph matrix elements '
    !
    IF (lscreen_tdbe .AND. scr_typ <= 1 ) THEN
      ALLOCATE(eps_rpa(nmode), STAT = ierr)
      IF (ierr /= 0) CALL errore('read_g2_tdbe', 'Error allocating eps_rpa', 1)
      eps_rpa(:)           = czero
    ENDIF
    !
    !! Write dielectric function
    IF (lscreen_tdbe .AND. scr_typ <= 1 .AND. mpime == ionode_id) THEN
      OPEN(UNIT = 400, FILE = 'eps_rpa.dat')
        DO iq = 1, nqtotfd ! loop over q-points 
          xq = xqfd(:, iq)
          CALL shift_q(xq, qcart)  ! shift q point to the 1st BZ
          IF (scr_typ == 0) CALL rpa_epsilon(xq, wfd(:, iq), nmode, epsi, eps_rpa)
          IF (scr_typ == 1) CALL tf_epsilon(xq, nmode, epsi, eps_rpa)
          WRITE(400, *) qcart(:)
          DO imode = 1, nmode
            WRITE(400, '(3E22.14)') wfd(imode, iq), eps_rpa(imode)
          ENDDO
        ENDDO
      CLOSE(400)
    ENDIF
    ...
    imelt = (nmode + 4) * nqtotfd  + (4 + 2 * nbndfs) * nkfs
    ...
    imelt = (upper_bnd - lower_bnd + 1) * MAXVAL(nqfs(:)) * nbndfs**2 * nmode
    CALL mem_size_eliashberg(2, imelt)
    !
    ALLOCATE(nnq(nkfs), STAT = ierr)
    IF (ierr /= 0) CALL errore('read_g2_tdbe', 'Error allocating nnq', 1)
    ALLOCATE(g2(lower_bnd:upper_bnd , MAXVAL(nqfs(:)), nbndfs, nbndfs, nmode), STAT = ierr)
        ...
        DO iq = 1, nqtotfd ! loop over q-points
          ...
          DO ik = 1, nks ! loop over k-points in the pool
            IF (ixkqf(ik+nnk, iq) > 0) THEN
              ...
              DO imode = 1, nmode ! loop over phonon modes
                     ...
                     DO jbnd = 1, nbndfs ! loop over jband's
                      IF (ABS(ekfs(jbnd, ixkqf(ik + nnk, iq)) - efm0) < fsthick) THEN
                         ...
                         IF (ik+nnk >= lower_bnd .AND. ik+nnk <= upper_bnd) THEN
                           IF (wfd(imode, iq) > eps_acustic) THEN
    ...
    !!
    USE kinds,          ONLY : DP
    USE epwcom,         ONLY : nkf1d, nkf2d, nkf3d
    ...
    !
      ...
      USE epwcom,        ONLY : mp_mesh_k, bte_phph, bte_elel, vme, &
                                lscreen_tdbe, scr_typ, assume_metal
      USE elph2,         ONLY : wf, wqf, xqf,adapt_smearing
      USE eliashbergcom, ONLY : ekfs, xkfs, wkfs, g2, ixkff,        &
                                ixkqf, ixqfs, nqfs,  memlt_pool
      USE tdbecom,       ONLY : enk_all, wkf_all, xkf_all, vnk_all, & 
      ...
      ENDIF
      IF (bte_elel) THEN
      DEALLOCATE(ie_e, STAT = ierr)
      IF (ierr /= 0) CALL errore('deallocate_tdbe','Error deallocating ie_e',1)
      ENDIF
    ...
    !
    !-----------------------------------------------------------------------
    SUBROUTINE kpoint_grid_tdbe(nrot, time_reversal, skip_equivalence, s, t_rev, &
                               nkc1, nkc2, nkc3, bztoibz, s_bztoibz,nkirr)
    !-----------------------------------------------------------------------
    !!  Modified by ymPan
    ...
    !!  EPW and the irr k points are not reordered because I am only interested in 
    !!  the BZtoIBZ map and the symmetry 
    !!
    USE kinds,            ONLY : DP
    USE division,         ONLY : fkbounds
    ...
    !! Symmetry matrice.
    INTEGER, INTENT(inout) :: s_bztoibz(nkc1 * nkc2 * nkc3)
    ...
    IF (ierr /= 0) CALL errore('kpoint_grid_tdbe', 'Error allocating nkspar', 1)
    ALLOCATE(xkg(3, nkr), STAT = ierr)
    IF (ierr /= 0) CALL errore('kpoint_grid_tdbe', 'Error allocating xkg', 1)
    ALLOCATE(wkk(nkr), STAT = ierr)
    IF (ierr /= 0) CALL errore('kpoint_grid_tdbe', 'Error allocating wkk', 1)
    equiv(:) = 0
    s_save(:) = 0
    !
    DO i = 1, nkc1
    ...
    IF (skip_equivalence) THEN
      CALL infomsg('kpoint_grid_tdbe', 'ATTENTION: skip check of k-points equivalence')
      wkk = 1.d0
    ELSE
      DO nk = 1, nkr
          ...
          DO ns = 1, nrot
            ...
            IF (in_the_list) THEN
              ...
              IF (n > nk .AND. equiv(n) == n) THEN
                ...
                s_save(n) = ns
              ELSE
                IF (equiv(n) /= nk .OR. n < nk) CALL errore('kpoint_grid_tdbe', &
    ...
    IF (ierr /= 0) CALL errore('kpoint_grid_tdbe', 'Error deallocating xkg', 1)
    DEALLOCATE(wkk, STAT = ierr)
    IF (ierr /= 0) CALL errore('kpoint_grid_tdbe', 'Error deallocating wkk', 1)
    DEALLOCATE(nkspar, STAT = ierr)
    IF (ierr /= 0) CALL errore('kpoint_grid_tdbe', 'Error deallocating nkspar', 1)
    !
    RETURN
    !-----------------------------------------------------------------------
    END SUBROUTINE kpoint_grid_tdbe
    !-----------------------------------------------------------------------
    !
    !-----------------------------------------------------------------------
    ...
    !
    USE kinds,            ONLY : DP 
    USE epwcom,           ONLY : bte_solver, bte_phph, bte_elel
    USE tdbecom,          ONLY : b_tab, c_tab, a_tab, nstg, felec,  &
                                 nphon, ie_ph, iph_e, iph_ph, ie_e, &
                                 nphon_pre
    USE elph2,            ONLY : nktotf, nqtotf, nbndfst
    ...
    CASE ('rk_4')
      ...
      a_tab(4, 3) = 1.0E0_DP
    CASE DEFAULT  
      CALL errore('allocate_tdbe', 'bte_solver is either euler, heun or rk_4', 1)
    END SELECT
    ...
    ENDIF
    IF (bte_elel) THEN
      allocate(ie_e(nbndfst, nktotf,nstg), STAT = ierr)
      IF (ierr /= 0) CALL errore('allocate_tdbe', 'Error allocating ie_e', 1)
    ENDIF
    ! Initialize
    felec(:, :) = 0.0E0_DP
    nphon(:, :) = 0.0E0_DP
    ...
    !!
    USE kinds,     ONLY : DP
    USE epwcom,    ONLY : system_2d, nbndsub
    ...
    USE mp_world,  ONLY : mpime
    USE elph2,     ONLY : L, do_cutoff_2D_epw, area
    !
    IMPLICIT NONE
    !
    !
...
!     IF (ierr /= 0) CALL errore('crystal_fmt_read', 'Error allocating w_centers', 1)
    ...
    !------------------------------------------------------------------------------
    USE kinds,     ONLY : DP
    USE epwcom,    ONLY : nbndsub, vme, lifc, nqc1, nqc2, nqc3
    USE constants_epw, ONLY : zero, czero, cone
    USE pwcom,     ONLY : ef
    USE elph2,     ONLY : chw, rdw,zstar, crrw, epsi,          &
    ...
    !------------------------------------------------------------------------------
    USE kinds,         ONLY : DP
    USE epwcom,        ONLY : nkf1d, nkf2d, nkf3d, nqf1d, nqf2d, nqf3d
    USE symm_base,     ONLY : nrot,s, t_rev, time_reversal, nsym
    USE eliashbergcom, ONLY : ixkff, xkfs, nkfs
    USE tdbecom,       ONLY : bztoibz_dg, s_bztoibz_dg, nkirr_dg,       &
    ...
    bztoibz_dg(:) = 0
    s_bztoibz_dg(:) = 0
    CALL kpoint_grid_tdbe(nsym, time_reversal, .FALSE., s, t_rev, nkf1d, nkf2d, &
                          nkf3d, bztoibz_dg, s_bztoibz_dg,nkirr_dg)
!    CALL kpoint_grid_tdbe(nsym, time_reversal, .FALSE., s, t_rev, nqfd1, nqfd2, nqfd3, bztoibzq_dg, s_bztoibzq_dg,nqirr_dg)
  ...
  !-----------------------------------------------------------------------
  SUBROUTINE bte_restart_read()
  !-----------------------------------------------------------------------
  !! Restart time-dependent simulation by reading the electron and phonon
  !! distribution 
  !-----------------------------------------------------------------------
  USE kinds,            ONLY : DP
  USE epwcom,           ONLY : bte_phph, lscreen_tdbe, scr_typ, &
                               assume_metal
  USE modes,            ONLY : nmodes
  USE io_global,        ONLY : stdout, ionode_id, meta_ionode
  USE io_var,           ONLY : iufelecrestart, iunphonrestart
  USE elph2,            ONLY : nqtotf, nktotf, nbndfst
  USE tdbecom,          ONLY : tstart, felec, nphon, &
                               nphon_pre, epsil_ipa
  USE modes,            ONLY : nmodes
  USE mp_world,         ONLY : mpime
  USE mp,               ONLY : mp_bcast
  ...
  tstart = 0.d0
    ...
    IF (mpime == ionode_id) THEN
      ...
      IF (ios /= 0) CALL errore('bte_restart_read', 'error opening file felec_restart', iufelecrestart)
      ...
      IF (ios /= 0) CALL errore('bte_restart_read', 'error opening file nphon_restart', iunphonrestart)
...
!       CLOSE(unit=iunphonrestart,STATUS = 'delete')
    ...
    !
    IF (bte_phph)   CALL mp_bcast(nphon_pre, ionode_id, inter_pool_comm) 
    !
    IF (lscreen_tdbe .AND. scr_typ > 1 .AND. (.NOT. assume_metal)) THEN
    ...
    !
    !-----------------------------------------------------------------------
    END SUBROUTINE bte_restart_read   
    !-----------------------------------------------------------------------
    !
    !-----------------------------------------------------------------------
    ...
    !-----------------------------------------------------------------------     
    USE kinds,            ONLY : DP
    USE epwcom,           ONLY : initial_type, initial_sigma, carrier_dynamic, &
                                 ef_c, ef_v, temp_el, temp_ph, eps_acustic,    &
                                 bte_phph, lscreen_tdbe, scr_typ, assume_metal
    USE constants_epw,    ONLY : kelvin2Ry, one, two, zero, ryd2ev
    USE modes,            ONLY : nmodes
    USE io_global,        ONLY : stdout, ionode_id, meta_ionode
    USE mp_global,        ONLY : inter_pool_comm, world_comm,npool, my_pool_id
    USE elph2,            ONLY : wf, nqtotf, nktotf, nbndfst, upper_bnd, lower_bnd
    USE tdbecom,          ONLY : tstart, enk_all, ef0, felec, nphon, nphon_pre
    USE mp,               ONLY : mp_bcast, mp_barrier, mp_sum
    !
    IMPLICIT NONE
    !
    ...
    WRITE(stdout, '(5x, a, f8.3, a/)' ) 'Initial distribution of phonon is simulated with BE distribution at ', & 
                                     temp_ph, ' K temperarue'
    IF (initial_type == 'FD') THEN
      inv_temp_el = one / (kelvin2Ry * temp_el)
      WRITE(stdout, '(5x, a, f8.3, a)' ) 'Initial distribution of electrons is simulated with FD distribution at ', &
      temp_el, ' K (temperarue)'
      IF (carrier_dynamic == 1) THEN   
        WRITE(stdout,'(5x,a)') 'Real time dynamics for electron, initial distribution determined by FD distribution'
        WRITE(stdout,'(5x,a,f8.6,a)') 'chemical potential of electron is', ef_c,' eV'
        DO ik = lower_bnd, upper_bnd
          DO ibnd = 1, nbndfst
            IF(enk_all(ibnd, ik) < ef0) CALL errore('set_initial_dist', 'Please keep only &
                                                    conduction bands in the energy window', 1)
            felec(ibnd,ik) = wgauss(-(enk_all(ibnd, ik) - (ef_c / ryd2ev)) * inv_temp_el, -99)
          ENDDO
        ENDDO
      ELSEIF (carrier_dynamic == 2) THEN
        WRITE(stdout,'(5x,a)') 'Real time dynamics for holes, initial distribution determined by FD distribution'
        WRITE(stdout,'(5x,a,f8.6,a)') 'chemical potential of hole is', ef_v,' eV'
        DO ik = lower_bnd, upper_bnd
          DO ibnd = 1, nbndfst
            IF(enk_all(ibnd, ik) > ef0) CALL errore('set_initial_dist', 'Please keep only &
                                                  valence bands in the energy window', 1)
            felec(ibnd,ik) = wgauss(-(enk_all(ibnd, ik) - (ef_v / ryd2ev)) * inv_temp_el,-99)
          ENDDO
        ENDDO
      ELSEIF (carrier_dynamic  == 3) THEN
        WRITE(stdout,'(5x,a)') 'Real time dynamics for electron and holes, initial distribution determined &
                                by FD distribution'
        WRITE(stdout,'(5x,a,f10.6,a)') 'chemical potential of electron is', ef_c,' eV'
        WRITE(stdout,'(5x,a,f10.6,a)') 'chemical potential of hole is', ef_v,' eV'
        DO ik = lower_bnd, upper_bnd
          DO ibnd = 1, nbndfst
        ...
        ENDDO
      ELSE
        CALL errore('set_initial_dist', 'carrier_dynamic could only be 1, 2 or 3', 1)
      ENDIF
    ELSEIF (initial_type == 'gaussian') THEN   
      IF (carrier_dynamic == 1) THEN
        WRITE(stdout,'(5x,a)') 'Real time dynamics for electron, initial distribtuion given by gaussian function.'
        WRITE(stdout,'(5x,a,f10.6,a)') 'initial energy of hot electron is', ef_c,' eV'
        WRITE(stdout,'(5x,a,f10.6,a)') 'gaussian spread for initial distribution is', initial_sigma, ' eV'
        DO ik = lower_bnd, upper_bnd
          DO ibnd = 1, nbndfst
            IF(enk_all(ibnd, ik) < ef0) CALL errore('set_initial_dist', &
              'Please keep only conduction bands in the energy window', 1)
            felec(ibnd,ik) = sqrtpi * w0gauss((enk_all(ibnd, ik) - (ef_c / ryd2ev)) / &
                             (initial_sigma / ryd2ev),0)
          ENDDO
        ENDDO
      ELSEIF (carrier_dynamic == 2) THEN
        WRITE(stdout,'(5x,a)') 'Real time dynamics for holes, initial distribtuion given by gaussian function.'
        WRITE(stdout,'(5x,a,f10.6,a)') 'initial energy of hot hole is', ef_v,' eV'
        WRITE(stdout,'(5x,a,f10.6,a)') 'gaussian spread for initial distribution is', initial_sigma,' eV'
        DO ik = lower_bnd, upper_bnd
          DO ibnd = 1, nbndfst
            IF(enk_all(ibnd, ik) > ef0) CALL errore('set_initial_dist', &
             ...
             felec(ibnd, ik) = 1.d0 - sqrtpi * w0gauss((enk_all(ibnd, ik) -  &
                              (ef_v / ryd2ev)) / (initial_sigma / ryd2ev), 0)
          ENDDO
        ENDDO 
      ELSEIF (carrier_dynamic  == 3) THEN
          ...
          DO ibnd = 1, nbndfst
            IF(enk_all(ibnd, ik) > ef0) THEN
              felec(ibnd, ik) = sqrtpi * w0gauss((enk_all(ibnd, ik) - (ef_c / ryd2ev)) / &
                               (initial_sigma / ryd2ev), 0)
            ELSE
              felec(ibnd, ik) = 1.d0 - sqrtpi * w0gauss((enk_all(ibnd, ik) - (ef_v / ryd2ev)) &
                               / (initial_sigma / ryd2ev), 0)
            ENDIF
          ENDDO
        ENDDO
      ...
      ENDIF
    ELSE
      CALL errore('set_initial_dist', 'initial_type is either FD or gaussian', 1)
    ENDIF
    CALL mp_sum(felec, inter_pool_comm)
    CALL mp_barrier(inter_pool_comm)
    inv_temp_ph = one / (kelvin2Ry * temp_ph)
    DO iq = 1, nqtotf
      DO nu = 1 , nmodes
    ...
    USE io_global,        ONLY : stdout, ionode_id, meta_ionode
    USE elph2,            ONLY : nqtotf, nktotf, nbndfst, upper_bnd, lower_bnd, &
                                 wf
    USE tdbecom,          ONLY : enk_all, ef0, felec, nphon, nphon_pre
    USE constants_epw,    ONLY : ryd2ev, ryd2mev
    USE modes,            ONLY : nmodes
    !
    IMPLICIT NONE
    !
    ...
    USE mp_world,         ONLY : mpime
    USE io_global,        ONLY : stdout, ionode_id, meta_ionode
    USE elph2,            ONLY : nqtotf, nktotf, nbndfst,wf, xqf
    USE tdbecom,          ONLY : enk_all, ef0, felec, nphon, nphon_pre, &
                                 xkf_all
    ...
    !-----------------------------------------------------------------------
    USE kinds,            ONLY : DP                               
    USE elph2,            ONLY : wf, wqf, xqf, xkf, nqtotf, nqf, nkf, nkqf, &
                                 nktotf, nkqtotf, etf, chw, ibndmax, area,  &
                                 qrpl, ibndmin, nbndfst,     lower_bnd,     &
                                 upper_bnd, nbndskip, wkf, rdw, wscache,    &
                                 adapt_smearing, zstar
    USE epwcom,           ONLY : nkf1, nkf2, nkf3, nqf1, nqf2, nqf3,        &
                                 eps_acustic, nbndsub,  fsthick, tstep,     & 
                                 use_ws, nqc1, nqc2, nqc3, nkc1, nkc2, nkc3,&
                                 efermi_read, fermi_energy, scissor, vme,   &
                                 bte_restart, lifc, ph_rta,   bte_phph,     &
                                 ph_read, facbroad,  mp_mesh_k, fixsym                               
    USE tdbecom,          ONLY : enk_all, wnuq_all, wkf_all, xkf_all,       &
                                 xqf_all, wqf_all, vnk_all, vnuq_all,       &
                                 totq, selecq, uf_all, ef0, unk_all,        &
                                 Amnk_all
    USE constants_epw,    ONLY : zero, czero, eps6, eps4, ryd2ev, ci, one,  &
                                 two, ryd2mev, twopi, ryd2ghz , bohr2ang     
    USE noncollin_module, ONLY : noncolin                        
    USE wan2bloch,        ONLY : hamwan2bloch, dynwan2bloch,vmewan2bloch,   &
    ...
    USE epwcom,           ONLY : lscreen_tdbe, scr_typ, assume_metal, mode_select                     
    !
    IMPLICIT NONE
    !
    INTEGER :: ierr
    ...
    !! BZ to IBZ mapping
    INTEGER :: s_bztoibz(nkf1 * nkf2 * nkf3)
    ...
    !! wannier centers
    REAL(KIND = DP), ALLOCATABLE :: cind(:, :)
    !! factor to dielectric function 0 or 1
    COMPLEX(KIND = DP) :: zqdotuq
    !
    ALLOCATE(w_centers(3, nbndsub), STAT = ierr)
    IF (ierr /= 0) CALL errore('crystal_fmt_read', 'Error allocating w_centers', 1)
  ...
  ! Center the WS at Gamma for electonic part, the phonon part and el-ph part
    ...
    ALLOCATE(wf(nmodes, nqf), STAT = ierr) ! defined in elph2, global
    IF (ierr /= 0) CALL errore('wanninterp', 'Error allocating wnuq_all', 1)
    ALLOCATE(etf(nbndsub, nkqf), STAT = ierr) ! defined in elph2, global
    ...
    IF (bte_phph .AND. (.NOT. ph_rta)) THEN
    ...
    !
    CALL kpoint_grid_tdbe(nsym, time_reversal, .FALSE., s, t_rev, nkf1, nkf2, nkf3, bztoibz, s_bztoibz, nkirr)
    ...
    DO ik = 1, nkqf
      ...
      IF (lscreen_tdbe .AND. scr_typ > 1 .AND. (2 * (ik / 2) /= ik) .AND. (.NOT. assume_metal)) THEN
        ...
        cufkk_all(:, :, ikk) = cufkk(:, :)
        IF (vme == 'wannier') THEN          
          CALL rrwan2bloch(nbndsub, nrr_k, cfac, dims, rrf_all(:, :, :, ikk))
        ENDIF
      ENDIF
      !! If we use adaptive broadening parameter in delta function, we need 
      !! additionally the band velocity
      IF (adapt_smearing) THEN
        CALL vmewan2bloch(nbndsub, nrr_k, irvec_k, cufkk, vmef, etf(:, ik), etf(:, ik), chw, cfac, dims)
        DO ibnd = 1, nbndsub
  ...
  !  CALL qwindow(.FALSE., nrr_k, dims, totq, selecq, irvec_r, ndegen_k, cufkk, cufkq, .TRUE.)
  !  print*, "the value of  bte_restart is", bte_restart
    IF (bte_restart) THEN
    ...
    CALL mp_barrier(inter_pool_comm) 
  !  DO iqq = 1, totq
    IF (bte_phph .AND. ph_read) THEN
      IF (ph_rta) CALL errore('wanninterp','ph_rta and ph_read can not be true at the same time',1)
      WRITE(stdout,'(5x, a)') 'read phonon frequencies and eigenmode from file'
      IF (mpime == ionode_id) THEN
      ...
      do iq = 1, nqtotf 
        do nu = 1, nmodes
          read(560,'(E22.14)') wf(nu, iq)
          do mu = 1, nmodes
            read(560,'(2E22.14)') uf_all(iq ,mu, nu)
          enddo
          !read(560,*)
        enddo
      enddo 
      close(560)
      IF (adapt_smearing) THEN
        WRITE(stdout,'(5x,a)') 'read phonon group velocity from file'
        OPEN(UNIT = 570, FILE = 'BTE.v_full',STATUS = 'unknown', FORM = 'formatted', IOSTAT = ios)
        IF (ios /= 0) CALL errore('wanninterp', 'error opening file BTE.v_full',1)
        do nu = 1, nmodes
          do iq = 1, nqtotf
            READ(570, *) vnuq_all(:, nu, iq)
          enddo
        enddo
        CLOSE(570)
        vnuq_all=vnuq_all / (twopi * ryd2ghz * bohr2ang * 100.d0) 
        !! Back to atomic unit
      ENDIF
      ENDIF
      CALL mp_bcast(wf,ionode_id, inter_pool_comm)
      CALL mp_bcast(uf_all, ionode_id, inter_pool_comm)
      IF (adapt_smearing) CALL mp_bcast(vnuq_all, ionode_id, inter_pool_comm)  
    ELSE
      DO iq = 1 , nqtotf
        xxq = xqf(:, iq)
        IF (.NOT. lifc) THEN
        ...
        ENDIF
        IF (bte_phph .and. (.not. ph_rta)) uf_all(iq, :, :) = uf0
        DO nu = 1, nmodes
          IF (w2(nu) > 0.d0) THEN
    ...
    ENDIF
    !
    WRITE(stdout,'(5x,a,f10.6)') 'threshold of phonon freqency(mev):',eps_acustic * ryd2mev
    !
    ...
    ENDIF
    !
    WRITE(stdout, *) mode_select, ph_rta
    IF (mode_select .AND. (.NOT. ph_rta)) THEN
      ALLOCATE(cind(nmodes, nqtotf), STAT = ierr)
      IF (ierr /= 0) CALL errore('wanninterp', 'Error deallocating cind', 1)
      cind = 0.d0
      WRITE(stdout, '(5x, a)') 'Selectively screen the EPC matrix element'
      DO iq = 1, nqtotf
        xxq(:) = xqf(:, iq)
        CALL shift_q(xxq, qcart) 
        qcart = qcart * (twopi / alat)
        DO nu = 1, nmodes
          zqdotuq = czero
          DO i = 1, nat 
             zbg(:) = qcart(1) * zstar(1, :, i) + qcart(2) * zstar(2, :, i) + qcart(3) * zstar(3, :, i)
             DO ir = 1, 3
                zqdotuq = zqdotuq + zbg(ir) * uf_all(iq, 3 * (i - 1) + ir, nu) / DSQRT(amass(ityp(i)))
             ENDDO
          ENDDO
          cind(nu, iq) = ABS(zqdotuq) ** 2
        ENDDO
      ENDDO
      !
      IF (mpime == ionode_id) THEN
         OPEN(UNIT = 566, FILE = 'mode_fac2.dat', STATUS = 'unknown', FORM = 'formatted', IOSTAT = ios)
         IF (ios /= 0) CALL errore('wanninterp', 'error opening file ph_freq',1)
         DO iq = 1, nqtotf
           DO nu = 1, nmodes
             WRITE(566,*) wf(nu, iq), cind(nu, iq)
           ENDDO
         ENDDO
         CLOSE(566)
      ENDIF
    ...
    !-----------------------------------------------------------------------
    USE kinds,            ONLY : DP
    USE elph2,            ONLY : wf, nkf, etf, wkf, nqtotf, lower_bnd, upper_bnd, &
                                 nbndskip, xqf, xkf, nktotf
    USE epwcom,           ONLY : nbndsub, vme
    USE cell_base,        ONLY : alat, omega
    USE tdbecom,          ONLY : unk_all, Amnk_all, epsil_ipa
    USE modes,            ONLY : nmodes
    USE mp_world,         ONLY : mpime
    USE io_global,        ONLY : stdout, ionode_id 
    USE mp,               ONLY : mp_barrier, mp_sum, mp_max
    USE mp_global,        ONLY : inter_pool_comm, npool, my_pool_id
    USE constants_epw,    ONLY : czero, eps5, zero, fpi, twopi, ci, cone
    USE rigid_epw,        ONLY : compute_umn_f
    USE noncollin_module, ONLY : noncolin
    USE grid,             ONLY : kpmq_map
    !
    IMPLICIT NONE
    ...
    USE epwcom,           ONLY : nbndsub, vme
    USE cell_base,        ONLY : alat, omega
    USE tdbecom,          ONLY : unk_all, Amnk_all, epsil_ipa, enk_all, felec, uf_all
    USE modes,            ONLY : nmodes
    USE mp_world,         ONLY : mpime
    USE io_global,        ONLY : stdout, ionode_id 
    USE mp,               ONLY : mp_barrier, mp_sum, mp_max
...
!         DO ibnd = 1, ivbm
!           DO jbnd = ivbm + 1, nbndsub
!             epsil_ipa(iq) = epsil_ipa(iq) - fac * fpi/ q2 / nktotf /   &
!             (etf_all(jbnd, ikq) - etf_all(ibnd, ik + lower_bnd - 1)) * &
!             ABS(bmatf(jbnd, ibnd) + qdotA(jbnd, ibnd)) ** 2
!           ENDDO
!         ENDDO
!         DO ibnd = ivbm + 1, nbndsub
...
!         ENDDO
    ...
    !-----------------------------------------------------------------------
    USE kinds,            ONLY : DP
    USE constants_epw,    ONLY : zero, one, two, twopi, hbar, ryd2ev,      &
                                 eps40, ryd2mev, czero
    USE epwcom,           ONLY : degaussw, tstep, tstep_write, teph_max,   &
                                 Ntstep, mp_mesh_k, fsthick, nkf1d, nkf2d, &
                                 nkf3d, eps_acustic, nkf1, nkf2, nkf3,     &
                                 nqf1, nqf2, nqf3 , facbroad, bte_solver,  &
                                 bte_phph, ph_rta,  bte_elel, dph,         &
                                 lscreen_tdbe, scr_typ, assume_metal,      &
                                 mode_select, ph_rta
    USE elph2,            ONLY : adapt_smearing, wf, xqf, wqf, nktotf,     &
                                 nqtotf, nbndfst, zstar
    USE tdbecom,          ONLY : nstg, a_tab, b_tab, c_tab, nkfsden,       &
    ...
    USE eliashbergcom,    ONLY : ixkff, g2, ibnd_kfs_to_kfs_all,           &
                                 ibnd_kfs_all_to_kfs
    USE mp_world,         ONLY : mpime
    USE io_global,        ONLY : stdout, ionode_id, meta_ionode
    USE cell_base,        ONLY : at, bg, alat, omega
    ...
    !! vnuq- vjk+q
    REAL(KIND = DP) :: cind(nmodes, nqtotf)
    !! factor to dielectric function 0 or 1
    COMPLEX(KIND = DP) :: zqdotuq
    !!
    LOGICAL  :: screen_g2
    ...
    dt = tstep * fs2sec
    WRITE(stdout,'(5x,a,a10,a)') 'Solving TDBE with ', bte_solver ,' method.'
      ...
      IF (mode_select .AND. (.NOT. ph_rta)) THEN
        ...
        DO iq = 1, nqtotf
          ...
          DO nu = 1, nmodes
            zqdotuq = czero
            DO iat = 1, nat 
               zbg(:) = qcart(1) * zstar(1, :, iat) + qcart(2) * zstar(2, :, iat) + qcart(3) * zstar(3, :, iat)
               DO ipol = 1, 3
                  zqdotuq = zqdotuq + zbg(ipol) * uf_all(iq, 3 * (iat - 1) + ipol, nu) / DSQRT(amass(ityp(iat)))
               ENDDO
            ENDDO
            cind(nu, iq) = ABS(zqdotuq) ** 2
          ENDDO
        ENDDO
        IF (mpime == ionode_id) THEN
           OPEN(UNIT = 566, FILE = 'mode_fac.dat', STATUS = 'unknown', FORM = 'formatted', IOSTAT = ios)
           IF (ios /= 0) CALL errore('wanninterp', 'error opening file ph_freq',1)
           DO iq = 1, nqtotf
             DO nu = 1, nmodes
               WRITE(566,*) wf(nu, iq), cind(nu, iq)
           ...
           CLOSE(566)
        ENDIF
!         CALL mp_bcast(cind,ionode_id, inter_pool_comm)
        DO iq = 1, nqtotf
          !xxq(:) = xqf(:, iq)
          !CALL shift_q(xxq, qcart)
          !qabs = (qcart(1)**2 + qcart(2)**2 + qcart(3)**2) * (twopi / alat) ** 2 
          !qabs = DSQRT(qabs)
          !qcart = qcart * (twopi / alat)
          DO nu = 1, nmodes
             !IF (cind(nu, iq) < 1.0E-18) THEN
             !IF (cind(nu, iq) < 1.0E-8) THEN
             IF (cind(nu, iq) < 1.0E-18 .AND. wf(nu, iq)*ryd2mev > 48) THEN
               epsil_ipa(nu, iq) = 1.d0
             ENDIF
          ENDDO
        ENDDO
      ENDIF 
    ...
    DO itt = 1, Ntstep
      ...
      IF (bte_phph .AND. MODULO(itt, dph) == 0) THEN
        IF (ph_rta) THEN
          CALL iphph_rta_calc()
        ELSE
          CALL iphph_calc()
      ...
      DO istg = 1, nstg
        ...
        ! IF (bte_elel) THEN
        !   felec(:,:) = felec(:,:) + dt*(ie_e(:,:,1)+ie_e(:,:,2))/2
        ! ENDIF
      ...
      IF (itt == 1) THEN
        ...
        ENDIF
      ENDIF
      IF (MODULO(itt, tstep_write) == 0) THEN
        WRITE(stdout, '(/5x, a, f15.6 , a)' ) 'Electron and phonon distributions at ',itt*tstep+tstart, ' fs are written to files'
        CALL write_dist(.false.,itt*tstep+tstart)
      ENDIF   
    ...
    CONTAINS
      ...
      ELSE
        ...
        eta = facbroad*0.5d0 * DSQRT(eta_tmp(1)**two + eta_tmp(2)**two + &

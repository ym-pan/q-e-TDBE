...
!      WRITE(stdout, '(5x, a, ES20.10)') 'Fermi level from nscf calculation (eV) = ', ef*ryd2ev
    ...
    !
      ...
      IF (lscreen_tdbe .AND. scr_typ > 1 .AND. (.NOT. assume_metal)) THEN
        ...
        IF (ierr /= 0) CALL errore('deallocate_tdbe', 'Error deallocating unk_all', 1)
        DEALLOCATE(epsil_ipa, STAT = ierr)
        IF (ierr /= 0) CALL errore('deallocate_tdbe', 'Error deallocating epsil_ipa', 1)
        IF (vme == 'wannier') THEN 
          DEALLOCATE(Amnk_all, STAT = ierr)
          IF (ierr /= 0) CALL errore('deallocate_tdbe', 'Error allocating Amnk_all', 1)
        ENDIF
      ENDIF
...
!       CLOSE(unit=iunphonrestart,STATUS = 'delete')
    ...
    USE elph2,            ONLY : wf, wqf, xqf, xkf, nqtotf, nqf, nkf, nkqf, &
                                 ...
                                 qrpl, ibndmin, nbndfst, cu, lower_bnd,     &
    ...
    USE epwcom,           ONLY : lscreen_tdbe, scr_typ, assume_metal                     
    !
    IMPLICIT NONE
    !
    INTEGER :: ierr
  ...
  !  DO iqq = 1, totq
    ...
    !-----------------------------------------------------------------------
    USE kinds,            ONLY : DP
    USE elph2,            ONLY : nkf, etf, wkf, nqtotf, lower_bnd, upper_bnd, &
    ...
    USE epwcom,           ONLY : nbndsub, vme
    USE cell_base,        ONLY : alat, omega
    USE tdbecom,          ONLY : unk_all, Amnk_all, epsil_ipa, enk_all, felec
    USE mp_world,         ONLY : mpime
    USE io_global,        ONLY : stdout, ionode_id 
    USE mp,               ONLY : mp_barrier, mp_sum, mp_max
    USE mp_global,        ONLY : inter_pool_comm, npool, my_pool_id
...
!         ENDDO
    ...
    IF (lscreen_tdbe .AND. scr_typ > 1 .AND. (.NOT. assume_metal) ) THEN 
      ...
      WRITE(stdout, '(5x, a , f16.10)') 'The maximum value of 1/epsilon(iq)**2 is : ', MAXVAL(epsil_ipa)
      WRITE(stdout, '(5x, a , f16.10)') 'The minimum value of 1/epsilon(iq)**2 is : ', MINVAL(epsil_ipa) 
    ENDIF
    DO itt = 1, Ntstep

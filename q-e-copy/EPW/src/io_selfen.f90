  ...
  CONTAINS
    ...
    !! 
    USE kinds,         ONLY : DP
    USE elph2,         ONLY : gtemp, nqtotf, lambda_all, wf, gamma_all, domega_all
    USE constants_epw, ONLY : ryd2ev, kelvin2eV, ryd2mev
    USE epwcom,        ONLY : nstemp, two_ef, temp_el
    ...
    IF (mpime == ionode_id) THEN
      ...
      DO itempphen = 1, nstemp
        ...
        IF (two_ef .AND. nstemp == 1) WRITE(tp, "(f8.3)") temp_el
        ...
        !! 
        filephlinewid = 'domega.phself.' // trim(adjustl(tp)) // 'K'
        OPEN(UNIT = linewidth_phself, FILE = filephlinewid)
        WRITE(linewidth_phself, '(a)') '# Phonon frequency and phonon lifetime in meV '
        WRITE(linewidth_phself, '(a)') '# Q-point  Mode   Phonon freq (meV)   Phonon linewidth (meV)'
        DO iqq = 1, nqtotf
          DO imode = 1, nmodes
            WRITE(linewidth_phself, '(i9,i6,E20.8,E22.10)') iqq, imode, &
                    ryd2mev * wf(imode, iqq),  ryd2mev * REAL(domega_all(imode, iqq, 1, itempphen))
          ENDDO
        ENDDO
        CLOSE(linewidth_phself)        
      ENDDO ! itempphen

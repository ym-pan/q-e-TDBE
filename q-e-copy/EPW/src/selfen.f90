  ...
  CONTAINS
    ...
    USE epwcom,        ONLY : nstemp, nbndsub, shortrange, fsthick, ngaussw, degaussw, &
                              ...
                              two_ef, ef_c, ef_v, temp_el, lorentz_gamma
    USE pwcom,         ONLY : ef
    ...
    REAL(KIND = DP) :: inv_electemp, efc_ry, efv_ry
    !
    ...
    IF (two_ef) THEN
      efc_ry = ef_c / ryd2ev
      efv_ry = ef_v / ryd2ev
      inv_electemp = one / (temp_el * kelvin2Ry)
    ENDIF
    !
    ...
    DO itemp = 1, nstemp ! loop over temperatures
      ...
      IF (iqq == 1) THEN
        ...
        !
        IF (two_ef) WRITE(stdout, '(5x, "two chemical potentials for conduction bands and valence bands")')
        !
      ENDIF
      !
      ! Fermi level
            ...
            DO imode = 1, nmodes
              DO ibnd = 1, nbndfst
                ...
                DO jbnd = 1, nbndfst
                  ...
                  IF (two_ef) THEN
                    IF (etf(ibndmin - 1 + jbnd, ikq) > ef0) THEN
                      wgkq = wgauss(-(ekq + ef0 - efc_ry ) * inv_electemp, -99)
                    ELSE 
                      wgkq = wgauss(-(ekq + ef0 - efv_ry ) * inv_electemp, -99)
                    ENDIF
                  ELSE
                  ...
                  IF (lorentz_gamma) THEN
                    w0g1 = eta_tmp / (etmp1 ** 2 + eta_tmp ** 2) / pi
                    w0g2 = eta_tmp / (etmp2 ** 2 + eta_tmp ** 2) / pi
                  ELSE 
    ...
    IF (iqq == totq) THEN
      ...
      DO itemp = 1, nstemp
        ...
        IF (two_ef) THEN
          WRITE(stdout, '(5x, a, f8.3, a)') "Phonon Temperature: ", gtemp(itemp) * ryd2ev / kelvin2eV, "K"
        ELSE
    ...
    USE elph2,      ONLY : epf17, ibndmax, ibndmin, etf, wkf, xqf, wqf, nkqf,   &
                           ...
                           gtemp, nktotf, adapt_smearing, sigmai_all, sigmar_all, &
                           zi_all, domega_all, dwf17
    USE mp,         ONLY : mp_barrier, mp_sum
    USE mp_global,  ONLY : inter_pool_comm
    USE constants_epw, ONLY : kelvin2eV, ryd2mev, ryd2ev, one, two, zero, eps4,   &
                              eps6, eps8, kelvin2Ry
    USE constants,  ONLY : pi
    USE epwcom,     ONLY : two_ef, ef_c, ef_v, lorentz_gamma, phlw_chp, temp_el, wfpt
    !
    IMPLICIT NONE
    !
    INTEGER, INTENT(in) :: iqq
    ...
    CHARACTER(LEN = 256) :: fileselfen
    !! File name for electron linewidth
    INTEGER :: ik
    !! Counter on the k-point index
    INTEGER :: ikk
    !! k-point index
    ...
    !! Eigen energy at k+q on the fine grid relative to the Fermi level
    REAL(KIND = DP) :: ekq2
    !! Eigen energy at k plus the phonon energy: enk+hw if expression 
    !! derived by Cheol-Hwan Park is used
    REAL(KIND = DP) :: wgkk, wgkk0
    !! Fermi-Dirac occupation factor $f_{nk}(T)$
    REAL(KIND = DP) :: wgkq, wgkq0, wgkqr
    ...
    REAL(KIND = DP) :: weightr, weightr0, weight_dw
    !! Real  part of the phonhon self-energy factor
    REAL(KIND = DP) :: w0g1
    !! Dirac delta at k for the imaginary part of $\Sigma$
    REAL(KIND = DP) :: w0g2
    !! Dirac delta at k+q for the imaginary part of $\Sigma$
    ...
    REAL(KIND = DP) :: domega(nmodes)
    !! domega is the real part of the phonon self-energy
    ...
    REAL(KIND = DP) :: domega_tmp(nmodes)
    !! Temporary value of domega for av.
    ...
    REAL(KIND = DP) :: inv_electemp, efc_ry, efv_ry
    REAL(KIND = DP) :: gamma_nk
    REAL(KIND = DP) :: dfnk, dfnk0
    !!
    INTEGER :: ikind, bndind
    REAL(KIND = DP) :: eik
    !
    ...
    IF (two_ef) THEN
      ...
      efv_ry = ef_v / ryd2ev
      inv_electemp = one / (temp_el * kelvin2Ry)
      ! IF (lorentz_gamma .AND. iq == 1) THEN 
      !   ALLOCATE(sigmai_all(nbndfst, nktotf, 1), STAT = ierr)
      !   IF (ierr /= 0) CALL errore('selfen_phon_q', 'Error allocating sigmai_all', 1)
      !   sigmai_all = zero
      !   IF (mpime == ionode_id) THEN
      !     fileselfen = 'linewidth.elself' 
      !     OPEN(UNIT = linewidth_elself, FILE = fileselfen)
      !     READ(linewidth_elself, *)
      !     READ(linewidth_elself, *)
      !     DO ik = 1, nktotf
      !       DO ibnd = 1, nbndfst
      !         READ(linewidth_elself, *) ikind, bndind, eik, sigmai_all(ibnd, ik, 1)
      !       ENDDO ! ibnd
      !     ENDDO ! ik
      !     CLOSE(linewidth_elself)
      !   ENDIF
      !   sigmai_all = sigmai_all / ryd2mev
    ...
    DO itemp = 1, nstemp
      ...
      DO ismear = 1, nsmear
              ...
              DO ibnd = 1, nbndfst
                ...
                IF (two_ef) THEN
                  IF (etf(ibndmin - 1 + ibnd, ikk) > ef0) THEN
                    wgkk = wgauss(-(ekk + ef0 - efc_ry ) * inv_electemp, -99)
                  ELSE
                    wgkk = wgauss(-(ekk + ef0 - efv_ry ) * inv_electemp, -99)
                  ENDIF 
                ELSE
                ...
                ENDIF
                !!
                wgkk0 = wgauss(-ekk * inv_eptemp, -99)
                !!
                !
                IF (wfpt) weight_dw = wkf(ikk) * (wgkk - wgkk0) / 2.d0
                !
                DO jbnd = 1, nbndfst
                  !
                  ...
                  ELSE
                    ...
                    IF (two_ef) THEN
                      IF (etf(ibndmin - 1 + jbnd, ikq) > ef0) THEN
                        wgkq = wgauss(-(ekq2 + ef0 - efc_ry ) * inv_electemp, -99)
                        wgkqr = wgauss(-(ekq + ef0 - efc_ry ) * inv_electemp, -99)
                      ELSE
                        wgkq = wgauss(-(ekq2 + ef0 - efv_ry ) * inv_electemp, -99)
                        wgkqr= wgauss(-(ekq + ef0 - efv_ry ) * inv_electemp, -99)
                      ENDIF 
                      !                   
                    ELSE 
                      wgkq = wgauss(-ekq2 * inv_eptemp, -99)
                      wgkqr = wgauss(-ekq * inv_eptemp, -99)
                      !
                    ...
                    ENDIF ! two_ef
                    !
                    wgkq0 = wgauss(-ekq * inv_eptemp, -99)
                    !
                    IF (lorentz_gamma) THEN 
                      weight = wkf(ikk) * (wgkk - wgkq) * degaussw0 / ((ekq - ekk - wq(imode)) ** 2 + degaussw0 ** 2) 
                    ELSE 
                      weight = pi * wkf(ikk) * (wgkk - wgkq) * w0gauss((ekq - ekk - wq(imode)) * inv_degaussw0, 0) * inv_degaussw0
                    ENDIF
                    !weightr =  wkf(ikk) * (wgkq - wgkk) * (ekq - ekk - wq(imode)) / ((ekq - ekk - wq(imode)) ** 2 + degaussw0 ** 2) &
                    !         - wkf(ikk) * (wgkq - wgkk) * (ekq - ekk ) / ((ekq - ekk) ** 2 + degaussw0 ** 2)
                    !weightr0 = wkf(ikk) * (wgkq0 - wgkk0) * (ekq - ekk - wq(imode)) / ((ekq - ekk - wq(imode)) ** 2 + degaussw0 ** 2) &
                    !         - wkf(ikk) * (wgkq0 - wgkk0) * (ekq - ekk ) / ((ekq - ekk ) ** 2 + degaussw0 ** 2)
                    weightr =  wkf(ikk) * (wgkqr - wgkk) *   (ekq - ekk - wq(imode)) / ((ekq - ekk - wq(imode)) ** 2 + degaussw0 ** 2) 
                    weightr0 = wkf(ikk) * (wgkq0 - wgkk0) * (ekq - ekk ) / ((ekq - ekk ) ** 2 + degaussw0 ** 2)
                    !
                  ENDIF ! delta_approx
                  ! 
                  !
                  ...
                  !
                  domega(imode) = domega(imode) + (weightr - weightr0) * g2 
                  !
                ENDDO ! jbnd
                !
                IF (wfpt) domega(imode) = domega(imode) + weight_dw * REAL(dwf17(ibnd, ibnd, imode, ik))* inv_wq(imode)
                !
                !! Add extra intraband contribution
                !!! Intraband contribution to phonon linewidth/ frequency renorm.
                !!! = g(ibnd, ibnd, imode, ik) * k-point weight * [f(E_k) - f(E_k+w_q)]*gamma_nk / [w_q**2 + gamma_nk ** 2]
                IF (ABS(xqf(1, iq)) < eps4 .AND. ABS(xqf(2, iq)) < eps4 &
                                         .AND. ABS(xqf(3, iq)) < eps4   &
                                         .AND. (.NOT. delta_approx)) THEN 
                
                   dfnk0 = -1.d0 * w0gauss(ekk * inv_eptemp, -99) * inv_eptemp
                   !
                   IF (two_ef) THEN
                     IF (etf(ibndmin - 1 + ibnd, ikk) > ef0) THEN
                       dfnk  = -1.d0 * w0gauss((ekk + ef0 - efc_ry) * inv_electemp, -99) * inv_electemp
                     ELSE
                       dfnk  = -1.d0 * w0gauss((ekk + ef0 - efv_ry) * inv_electemp, -99) * inv_electemp
                     ENDIF
                   ELSE        
                     dfnk  = -1.d0 * w0gauss(ekk * inv_electemp, -99) * inv_electemp
                   ENDIF
                   g2 = (ABS(epf17(ibnd, ibnd, imode, ik))**two) * inv_wq(imode) * g2_tmp(imode)
                   dfnk  = dfnk *  wkf(ikk)
                   dfnk0 = dfnk0 * wkf(ikk)
                   domega(imode) = domega(imode) + g2 * (dfnk - dfnk0)
                !   ekq = ekk + wq(imode)
                !   IF (two_ef) THEN
                !     wgkq = wgauss(-ekq * inv_electemp, -99)
                !   ELSE
                !     wgkq = wgauss(-ekq * inv_eptemp, -99)
                !   ENDIF
                !   !
                !   IF (lorentz_gamma) THEN
                !     !
                !     gamma_nk = sigmai_all(ibnd, ik + lower_bnd - 1, itemp)
                !     weight =  wkf(ikk) * (wgkk - wgkq) * gamma_nk / (gamma_nk ** 2 + wq(imode) ** 2)
                !   ELSE
                !     weight =  pi * wkf(ikk) * (wgkk - wgkq) * w0gauss(wq(imode) * inv_degaussw0, 0) * inv_degaussw0
                !   ENDIF

                !   g2 = (ABS(epf17(ibnd, ibnd, imode, ik))**two) * inv_wq(imode) * g2_tmp(imode)
                !   gamma(imode) = gamma(imode) + weight * g2
                ENDIF !! Add intraband contribution to phonon linewidth
                ! !!
              ENDDO   ! ibnd
        ...
        CALL mp_sum(fermicount, inter_pool_comm)
        CALL mp_sum(domega, inter_pool_comm)
        CALL mp_barrier(inter_pool_comm)
        !
        ...
        DO imode = 1, nmodes
          ...
          WRITE(stdout, 102) imode, lambda_all(imode, iq, ismear, itemp), &
                             ...
                             ryd2mev * domega_all(imode, iq, ismear, itemp), ryd2mev * wq(imode)
    ...
    102 FORMAT(5x, 'lambda___( ', i3, ' )=', f15.6, '   gamma___=', f15.6, ' meV', '   domega___=', f15.6, ' meV' ,'   omega=', f12.4, ' meV')

...
PROGRAM diff_sca
  ...
  !
  LOGICAL                  :: phT_neq
  INTEGER                  :: nq1_neq, nq2_neq, nq3_neq
  CHARACTER(LEN = 256)     :: flphneq
  REAL(DP),    ALLOCATABLE :: T_neq(:, :)
  !
  ...
  ! mz_b we add the inputs for diffuse scattering
       ...
       &           full_phonon, atmsf_a, atmsf_b, nksf1, nksf2, nksf3, eps2, phT_neq, nq1_neq, &
       &           nq2_neq, nq3_neq, flphneq
  ...
  IF (ionode) CALL input_from_file ( )
     ...
     CALL mp_bcast(phT_neq, ionode_id, world_comm)
     CALL mp_bcast(nq1_neq, ionode_id, world_comm)
     CALL mp_bcast(nq2_neq, ionode_id, world_comm)
     CALL mp_bcast(nq3_neq, ionode_id, world_comm)
     CALL mp_bcast(flphneq, ionode_id, world_comm)
     ! 
...
20                  IF (ios /=0) CALL errore('disca',&
     ...
     ! mz_b
     !! IF the mode-resolved temperature is read from input (Y.Pan)
     IF (.NOT. phT_neq) THEN 
        nq1_neq = 1
        nq2_neq = 1
        nq3_neq = 1
     ENDIF
     ALLOCATE(T_neq(3*nat, nq1_neq * nq2_neq* nq3_neq))
     T_neq = T
     IF (phT_neq) THEN  
        IF (ionode) THEN 
          OPEN(unit = 88, file = flphneq, status = 'unknown', form = 'formatted')
          DO j = 1, nq1_neq * nq2_neq * nq3_neq
            DO i = 1, 3* nat
              READ(88, *) T_neq(i, j) 
            ENDDO
          ENDDO
          CLOSE(88)
        ENDIF
        CALL mp_bcast(T_neq, ionode_id, world_comm)
     ...
     IF (disca) CALL diffuse_scattering(nq, nq_super, nq_strft, nat, ntyp, amass, ityp, q_nq_zg, & 
                         ...
                         nrots, kres1, kres2, kmin, kmax, col1, col2, Np, phT_neq, nq1_neq, &
                         nq2_neq, nq3_neq, T_neq)
     !mz_e
     ! 
     !
...
SUBROUTINE diffuse_scattering(nq, nq_super, nq_strft, nat, ntyp, amass, ityp, q, w2, &
  ...
  LOGICAL, INTENT(IN)          :: phT_neq
  INTEGER, INTENT(IN)          :: nq1_neq, nq2_neq, nq3_neq
  REAL(DP), INTENT(IN)         :: T_neq(3 * nat, nq1_neq* nq2_neq* nq3_neq)
  !
  ! local
  !
  ...
  DO qp =  lower_bnd, upper_bnd ! 1, nq
    IF (phT_neq) THEN 
      CALL map_qpoints(q(:, qp), nq1_neq, nq2_neq, nq3_neq, qind)
      Tqneq(:) = T_neq(:, qind)
    ENDIF
    DO i = 1, nat3 
      ...
      ELSE
          ...
          l_q(i, qp) = SQRT(hbar / ph_w(i, qp) / 2.0d0 / AMU_SI / ang**2.0d0) & 
                     * SQRT(1.0d0 + 2.0d0 / (EXP(hbar * ph_w(i,qp)/ (K_BOLTZMANN_SI * Tqneq(i))) - 1)) 
          ELSE
          l_q(i, qp) = SQRT(hbar / ph_w(i, qp) / 2.0d0 / AMU_SI / ang**2.0d0) & 
...
END SUBROUTINE
!!
SUBROUTINE map_qpoints(q_cryst, nq1, nq2, nq3, qind)
!! Knowing the coordinate of a point, reduce it to the
!! grid of TDBE simulation and return the index 
USE kinds,      ONLY : dp
IMPLICIT NONE
! input
...
!
DO i = 1, 3
  qshift(i) = q_cryst(i) - FLOOR(q_cryst(i))
ENDDO
iq1 = MODULO(NINT(qshift(1) * nq1), nq1)
iq2 = MODULO(NINT(qshift(2) * nq2), nq2)
iq3 = MODULO(NINT(qshift(3) * nq3), nq3)
!
qind = iq1 * nq2 * nq3 + iq2 * nq3 + iq3 + 1
! 
RETURN
END SUBROUTINE map_qpoints

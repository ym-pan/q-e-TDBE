...
!------------------------------
  ...
  INTEGER                 :: dk_cry(3,3)
  !
  NAMELIST / inputpp / prefix, outdir, calculation, dipole_fd, nk1, nk2, nk3, dk_cry
...
!
  ...
  CALL mp_bcast( dipole_fd, ionode_id, intra_image_comm )
  ...
  CALL mp_bcast( dk_cry, ionode_id, intra_image_comm )
  !
...
!-----------------------------------------------------------------------------
SUBROUTINE eps_calc ( intersmear,intrasmear, nbndmin, nbndmax, shift, metalcalc , &
                      nspin, dipole_fd, nk1, nk2, nk3, dk_cry)
  !-----------------------------------------------------------------------------
  !
  USE kinds,                ONLY : DP
  ...
  LOGICAL,         INTENT(in)::  dipole_fd
  INTEGER,         INTENT(in)::  nk1, nk2, nk3
  INTEGER,         INTENT(in)::  dk_cry(3,3)
  !
  ! local variables
  !
...
!
    ...
    epsi(:,:)  = 0.0_DP
    ieps(:,:)  = 0.0_DP
    ! IF (ionode .AND. dipole_fd) THEN
    !     OPEN(unit=10, file='gvect.dat')
    !     DO ik = 1, nks
    !         write(10, *) 'kpt', ik, '= ', xk(1, ik), xk(2, ik), xk(3,ik)
    !         write(10, *) 'ngk(ik) = ', ngk(ik)
    !         DO ig = 1, ngk(ik)
    !             write(10, *) g(1,igk_k(ig,ik)), g(2,igk_k(ig,ik)), g(3,igk_k(ig,ik))
    !         ENDDO
    !     ENDDO
    !     CLOSE(10)
    ! ENDIF
    ...
    DO ik = 1, nks
        ...
        !
        IF (dipole_fd) THEN
            CALL dipole_calc2( ik, dipole_aux, metalcalc , nbndmin, nbndmax, nk1, nk2, nk3, dk_cry)
            dipole(:,:,:)= REAL( dipole_aux(:,:,:) * conjg(dipole_aux(:,:,:)), DP ) / tpiba2
        ELSE
                    ...
                    IF ( focc(iband1,ik) >= 0.5d-4*full_occ ) THEN
                        ...
                        DO iw = 1, nw
                            ...
                            epsi(:,iw) = epsi(:,iw) + dipole(:,iband1,iband2) * intersmear * w* &
                                                    ...
                                                    (( (etrans**2 -w**2 )**2 + intersmear**2 * w**2 )* etrans )* -1.0 * (etrans / RYTOEV) ** 2
...
!--------------------------------------------------------------------
SUBROUTINE dipole_calc2( ik, dipole_aux, metalcalc, nbndmin, nbndmax,&
                         nk1, nk2, nk3, dk_cry )
  !------------------------------------------------------------------
  !
  USE kinds,                ONLY : DP
  USE wvfct,                ONLY : nbnd, npwx
  USE wavefunctions,        ONLY : evc
  USE klist,                ONLY : xk, ngk, igk_k, nks, nkstot
  USE gvect,                ONLY : ngm, g
  USE io_files,             ONLY : restart_dir
  USE pw_restart_new,       ONLY : read_collected_wfc
  USE grid_module,          ONLY : focc, full_occ
...
USE io_global,            ONLY : ionode, stdout
  ...
  !
  ! local variables
  COMPLEX(DP), ALLOCATABLE  :: evckq(:,:)
  INTEGER     :: iband1,iband2,ig,npw, npwkq, ig2
  INTEGER     :: ipol, jpol
  INTEGER     :: npw_min
  COMPLEX(DP) :: caux
  REAL(DP)    :: dqnorm, dq(3)
  ...
  ! read wfc for the given kpt
  !
  IF (nks /= nkstot) CALL errore('dipole_calc2','npool parallization not supported in this subroutine', 1)
  !
  xxk(:) = xk(:, ik)
  ...
  DO ipol = 1, 3
    dq(1) = REAL(dk_cry(ipol, 1)) / REAL(nk1)
    dq(2) = REAL(dk_cry(ipol, 2)) / REAL(nk2)
    dq(3) = REAL(dk_cry(ipol, 3)) / REAL(nk3)
    CALL find_ikq(xxk, dq, nk1, nk2, nk3, ikq) 
    CALL cryst_to_cart(1, dq, bg, 1)
    !
    DO jpol = 1, 3
      IF (jpol == ipol) CYCLE
      IF (ABS(dq(jpol)) > 1e-3) CALL errore('dipole_calc2', 'dk not along the axis', 1)
    ENDDO
    !
    dqnorm = SQRT(dq(1) ** 2 + dq(2) ** 2 + dq(3) ** 2)
    !
    CALL read_collected_wfc ( restart_dir(), ikq, evckq )
    npw = ngk(ik)
    npwkq = ngk(ikq)
    ! npw_min = MIN(npw, npwkq)
    !
    DO iband2 = nbndmin,nbndmax
      IF ( focc(iband2,ik) <  full_occ) THEN
          DO iband1 = nbndmin,nbndmax
              ...
              IF ( focc(iband1,ik) >= 0.5e-4*full_occ ) THEN
                  ...
                  DO ig=1,npw
                    ...
                    DO ig2 = 1, npwkq
                      !
                      !!!!!! DEBUG
                      IF (g(1,igk_k(ig,ik)) /= g(1,igk_k(ig2,ikq)) .OR. &
                          g(2,igk_k(ig,ik)) /= g(2,igk_k(ig2,ikq)) .OR. &
                          g(3,igk_k(ig,ik)) /= g(3,igk_k(ig2,ikq))  ) CYCLE
                          ! write(stdout, *) "g vectors for 2 neigboring k points are not consistent"
                          ! write(stdout, *) ig
                          ! write(stdout, *) ik, ikq 
                          ! write(stdout, *) g(:, igk_k(ig,ik))
                          ! write(stdout, *) g(:, igk_k(ig,ikq))
                          ! CALL errore('dipole_calc2', 'g vectors for 2 neigboring k points are not consistent',1)
                      ! ENDIF
                      !!!!!!
                      caux= conjg(evckq(ig,iband1))*evc(ig,iband2)
                      find_gvect = .TRUE.
                      !
                      ! Non collinear case
                      IF ( nspin == 4 ) THEN
                          caux = caux + conjg(evckq(ig+npwx,iband1))*evc(ig+npwx,iband2)
                      ENDIF
                      !
                      dipole_aux(ipol,iband1,iband2) = dipole_aux(ipol,iband1,iband2) + caux / dqnorm
                      !
                      IF (find_gvect) EXIT
                    ENDDO
                  ENDDO
  ...
  CALL stop_clock( 'dipole_calc' )
  !
END SUBROUTINE dipole_calc2


SUBROUTINE find_ikq(xxk, dq, nk1, nk2, nk3, ikq)
  USE kinds,                ONLY : DP
  !
  IMPLICIT NONE 
  !
  ...
  zz = xkq(3) * nk3
  !
  in_the_list = ABS(xx - NINT(xx)) <= 1e-5 .AND. &
                ABS(yy - NINT(yy)) <= 1e-5 .AND. &
                ABS(zz - NINT(zz)) <= 1e-5
  IF (.NOT. in_the_list) CALL errore('find_ikq', 'k+dk does not fall on k-grid', 1)
  DO ib = -2, 0
    IF (NINT(xx) < ib * nk1) xx = xx + (-ib + 1) * nk1
    IF (NINT(yy) < ib * nk2) yy = yy + (-ib + 1) * nk2
    IF (NINT(zz) < ib * nk3) zz = zz + (-ib + 1) * nk3
  ENDDO
  DO ib = 2, 1, -1
    IF (NINT(xx) >= ib * nk1) xx = xx - ib * nk1
    IF (NINT(yy) >= ib * nk2) yy = yy - ib * nk2
    IF (NINT(zz) >= ib * nk3) zz = zz - ib * nk3
  ENDDO
  ikq = NINT(xx) * nk2 * nk3 + NINT(yy) * nk3 + NINT(zz) + 1
  RETURN
END SUBROUTINE find_ikq
